...
def build_network(net_name, ae_net=None, loss="normal"):
    ...
    if net_name == 'fmnist_LeNet':
        net = FashionMNIST_LeNet(loss=loss)
...
def build_autoencoder(net_name, loss="normal"):
    ...
    if net_name == 'mnist_LeNet':
        ae_net = MNIST_LeNet_Autoencoder(loss=loss)

    if net_name == 'mnist_DGM_M1M2':
        ae_net = VariationalAutoencoder([1*28*28, 32, [128, 64]])

    if net_name == 'fmnist_LeNet':
        ae_net = FashionMNIST_LeNet_Autoencoder(loss=loss)

    if net_name == 'fmnist_DGM_M1M2':
        ae_net = VariationalAutoencoder([1*28*28, 64, [256, 128]])

    if net_name == 'cifar10_LeNet':
        ae_net = CIFAR10_LeNet_Autoencoder(loss=loss)
