...
class MVTec_Dataset(TorchvisionDataset):
    ...
    def __init__(self, root: str, normal_class: int = 5, normalize: bool = False,
        ...
        self.train_set = MVTec(root=self.root + '/mvtec_anomaly_detection/' + classes[normal_class] + '/train',
                               transform=train_transform)
        
        self.test_set = MVTec(root=self.root + '/mvtec_anomaly_detection/' + classes[normal_class] + '/test',
                              transform=test_transform, blur_outliers=blur_outliers, blur_std=blur_std)
...
class MVTec(ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        return sample, target, target*-1, index
